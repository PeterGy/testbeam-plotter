...
def convolution_function( energy, gauss_amplitude=1, gauss_mean=3, gauss_width=0.1, distortion=1): 
    # print(gauss_amplitude, gauss_mean, gauss_width, )
    pure_signal = landau( energy, distortion )
...
def analyse_bar(layer,strip,end):
    ...
    deduced_signal = landau( energy, 1 )
    ...
    plt.plot( energy-3, deduced_noise,  linestyle='solid', color='lime', label='Deduced r/o response', zorder=1 )
    plt.plot( energy_extended, convolution_fit, linestyle='solid', color='cyan', label='Fit of data', zorder=1)
