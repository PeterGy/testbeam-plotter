...
inputFile2=TFile(sys.argv[2], "read")
...
c.cd()
hist = ROOT.TH2F('Map', "Difference between average SiPM ADCs",40, 0.5, 40.5, 12, -0.5, 11.5)
adcCountMap = zeros((40,12))
adcSumMap = zeros((40,12))
adcCountMap2 = zeros((40,12))
adcSumMap2 = zeros((40,12))


allData=inputFile.Get('ntuplizehgcroc').Get("hgcroc") #
allData2=inputFile.Get('ntuplizehgcroc').Get("hgcroc") #
...
for t in allData2 : #for timestamp in allData
    realChannel = FpgaLinkChannel_to_realChannel([t.fpga,t.link,t.channel])
    if realChannel != None:
        LayerBarSide = realChannel_to_SipM(realChannel)
        if LayerBarSide[2]==1: LayerBarSide[0] +=20
        adcCountMap2[LayerBarSide[0],LayerBarSide[1]] +=1 
        adcSumMap2[LayerBarSide[0],LayerBarSide[1]] +=t.adc

#fills a ROOT histogram with the sipm averages
adcCountMap[adcCountMap == 0 ] = 1 
adcCountMap2[adcCountMap2 == 0 ] = 1 
for i in range(40):
    for j in range(12):
        hist.Fill(i+1,j,adcSumMap[i,j]/adcCountMap[i,j]-adcSumMap2[i,j]/adcCountMap2[i,j])        
